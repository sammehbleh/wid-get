...
export default function WidgetFrame({ id, title, Component, editMode, grow, onHide }) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex flex-col ${grow ? "min-h-0 flex-1" : ""}`}
    >
      {editMode && (
      ...
      <Component className={grow ? "min-h-0 flex-1" : ""} />
    </div>
  );
}
